...
void LC_Vector2D_MulScaler(LC_Vector2D vec2, const float s) {
    vec2[0] *= s;
    vec2[1] *= s;
}

void LC_Vector3D_MulScaler(LC_Vector3D vec3, const float s) {
    vec3[0] *= s;
    vec3[1] *= s;
    vec3[2] *= s;
}

void LC_Vector4D_MulScaler(LC_Vector4D vec4, const float s) {
    vec4[0] *= s;
    vec4[1] *= s;
    vec4[2] *= s;
    vec4[3] *= s;
...
void LC_Vector2D_DivScaler(LC_Vector2D vec2, const float s) {
    const float divisor = 1.0f / s;
    vec2[0] *= divisor;
    vec2[1] *= divisor;
}

void LC_Vector3D_DivScaler(LC_Vector3D vec3, const float s) {
    const float divisor = 1.0f / s;
    vec3[0] *= divisor;
    vec3[1] *= divisor;
    vec3[2] *= divisor;
...
void LC_Vector4D_DivScaler(LC_Vector4D vec4, const float s) {
    const float divisor = 1.0f / s;
    vec4[0] *= divisor;
    vec4[1] *= divisor;
    vec4[2] *= divisor;
...
void LC_Vector2D_Normalize(LC_Vector2D vec2) {
    const float magnitude = LC_Vector2D_Magnitude(vec2);
    const float reciprocalOfSqrt = 1 / magnitude;
    LC_Vector2D_MulScaler(vec2, reciprocalOfSqrt);
}

void LC_Vector3D_Normalize(LC_Vector3D vec3) {
    const float magnitude = LC_Vector3D_Magnitude(vec3);
    const float reciprocalOfSqrt = 1 / magnitude;
    LC_Vector3D_MulScaler(vec3, reciprocalOfSqrt);
}

void LC_Vector4D_Normalize(LC_Vector4D vec4) {
    const float magnitude = LC_Vector4D_Magnitude(vec4);
    const float reciprocalOfSqrt = 1 / magnitude;
    LC_Vector4D_MulScaler(vec4, reciprocalOfSqrt);
}
...
void LC_Matrix2D_InitializeF(const float n00, const float n01,
...
void LC_Matrix3D_InitializeF(const float n00, const float n01, const float n02,
...
void LC_Matrix4D_InitializeF(const float n00, const float n01, const float n02, const float n03,
...
void LC_Matrix2D_MulMatrix2D(LC_Matrix2D a, LC_Matrix2D b, LC_Matrix2D destination) {
    LC_Matrix2D_InitializeF(a[0][0] * b[0][0] + a[1][0] * b[0][1],
...
void LC_Matrix3D_MulMatrix3D(LC_Matrix3D a, LC_Matrix3D b, LC_Matrix3D destination) {
    LC_Matrix3D_InitializeF(a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2],
...
void LC_Matrix4D_MulMatrix4D(LC_Matrix4D a, LC_Matrix4D b, LC_Matrix4D destination) {
    LC_Matrix4D_InitializeF(a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2] + a[3][0] * b[0][3],
